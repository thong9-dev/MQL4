   ...
   if(Net_Trades>0 && Net_Lots!=0)
     {
      ...
      distance_BE=(BE/(MathAbs(Net_Lots*MarketInfo(Symbol(),MODE_TICKVALUE)))*MarketInfo(Symbol(),MODE_TICKSIZE));
      if(Net_Lots>0)
        {
   ...
   if(Net_Trades>0 && Net_Lots==0)
     {
      ...
      distance_BE=(BE/((MarketInfo(Symbol(),MODE_TICKVALUE)))*MarketInfo(Symbol(),MODE_TICKSIZE));
      Average_Price=Bid-distance;
      BE_Price=Average_Price-distance_BE;
     }
   ObjectDelete("Average_Price_Line_"+Symbol());
