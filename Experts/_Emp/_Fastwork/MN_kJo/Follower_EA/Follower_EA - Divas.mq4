...
extern double                 exTimeClose       =  7.30;         //TimeClose Minute
extern double                 exProfitC         =  2.5;
int    eaTimeClose =-1;
extern int                    exMaxOrders       =  33;      //MaxOrders
extern int                    exSteps           =  200;     //Steps (Point)
...
extern string                 exLine6=" ------------------------------------- ";// ----------
extern bool       exUseMA     =true;
extern bool       exUseMACut  =false;
//---
int ticket;
static int BARS;
//+------------------------------------------------------------------+
      ...
      if((ex2Lot_C!=-2 && exLot_Mode==LOT_Dynamic) || (exLot_Mode==LOT_Static))
        {
         ...
         bool A=Active_Hold>=exProfit;
         bool Z=A;

         if(Z)
           {
            //int MODE=(ActiveBuy_Lot>ActiveSell_Lot)?OP_BUY:OP_SELL;

            if(LastOrderTimeHold_Limit(OP_BUY)||LastOrderTimeHold_Limit(OP_SELL))
              {
         ...
         if(Price_Origin!=-1)
           {
            ...
            double loppSteps=(ActiveBuy!=0)?exSteps:exSteps/2;
            //loppSteps=0;
            double LastOrderPrice_Buy=(ActiveBuy!=0)?LastOrderPrice(OP_BUY):Price_Origin;
            double LastOrderPrice_BuyLine=LastOrderPrice_Buy+(loppSteps*Point);

            HLineCreate(0,"D_Buy",0,LastOrderPrice_BuyLine,clrRoyalBlue,STYLE_SOLID,1,true,false,0);
            //---
            bool cntMNG1=(ActiveBuy!=0&&Active_Hold<exProfit*exProfitC*(-1))
                         ||ActiveBuy==0
                         ||ActiveBuy<=ActiveSell;
            //
            bool maMNG1=true;
            //if(ActiveBuy!=0)
            maMNG1=((exUseMA&&(vMA_Fast_1>=vMA_Slow_1 && ((exUseMACut&&vMA_Fast_2<vMA_Slow_2)||!exUseMACut)))||!exUseMA);
            //---

            if(
               (ActiveBuy<exMaxOrders)
               && (cntMNG1)               //exProfit
               && (Bid>LastOrderPrice_BuyLine)
               && maMNG1
            ) //Fast State up
              {
               ticket=OrderSend(Symbol(),OP_BUY,Order_LotsMultiply(Active,LastOrderPrice_Buy,Ask),Ask,exSlipepage,0,0,exComments,exMagicnumber,0,clrBlue);
              }
            //--------------------------------------------------------------------------------
            loppSteps=(ActiveSell!=0)?exSteps:exSteps/2;
            //loppSteps=0;
            double LastOrderPrice_Sell=(ActiveSell!=0)?LastOrderPrice(OP_SELL):Price_Origin;
            double LastOrderPrice_SellLine=LastOrderPrice_Sell-(loppSteps*Point);

            HLineCreate(0,"D_Sell",0,LastOrderPrice_SellLine,clrTomato,STYLE_SOLID,1,true,false,0);
            //---
            bool cntMNG2=(ActiveSell!=0&&(Active_Hold<exProfit*exProfitC*(-1)))
                         ||(ActiveSell==0)
                         ||(ActiveBuy>=ActiveSell);
            //
            bool maMNG2=true;
            //if(ActiveSell!=0)
            maMNG2=((exUseMA&&(vMA_Fast_1<=vMA_Slow_1&&((exUseMACut&&vMA_Fast_2>vMA_Slow_2)||!exUseMACut)))||!exUseMA);
            //---

            if(
               (ActiveSell<exMaxOrders)
               && (cntMNG2)
               && (Ask<LastOrderPrice_SellLine)
               && maMNG2
            )//Fast State down
              {
               ticket=OrderSend(Symbol(),OP_SELL,Order_LotsMultiply(Active,LastOrderPrice_Sell,Bid),Bid,exSlipepage,0,0,exComments,exMagicnumber,0,clrRed);
              }
           }
         //---
...
//int exMode=1;
   ...
   if(exLot_Mode==LOT_Dynamic)
     {
      Print(__FUNCTION__+"|Raw :: "+string(Dy_Last)+" "+string(Dy_Now));
