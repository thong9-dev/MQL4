...
extern bool                    exUseLimitLoss =true;
extern double                  exLimitLoss  =5;//USD
...
extern int                     exMaxOrders    = 2;
extern int                     exSteps        = 300;
            ...
            if(MarketInfo(Symbol(),MODE_SPREAD)<=exSpreadLimit && ActiveBuy==0 && ActiveSell==0)
              {
               ...
               if((vMA_Fast_1<=vMA_Slow_1 && vMA_Fast_2>vMA_Slow_2)||(vMA_Fast_1>=vMA_Slow_1 && vMA_Fast_2<vMA_Slow_2))
                 {
...
//int exMode=1;
   ...
   Print(__FUNCTION__+"|"+LotmodeToString()+" :: "+R);
...
double LastOrderPrice(int _type)
  {
   double _price=(_type==OP_BUY)?9999999:0;
...
void ShowDataComm()
  {
   if(IsTesting())
     {
      string CMM="";
      CMM+="\n"+"ACCOUNT_BALANCE : "+string(AccountInfoDouble(ACCOUNT_BALANCE));
      CMM+="\n ---";
      Comment(CMM);
     }
  }
void ShowDataPanel()
  {
   ...
   ObjectSetInteger(0,objEA_,OBJPROP_YSIZE,350);
...
//----------------------------------------------------------------------------------------------
   objLabel("Lot@10",yy,xx+=xx_step,"Verdana",7,clrWhiteSmoke,
            "Sum Lots Buy ::");
//
...
//         DoubleToStr(ActiveBuy_Hold,2));
   ...
   objEdit("Lot@11",0,DoubleToStr(ActiveBuy_Lot,2)
           ...
           ,clrNumberValue(ActiveBuy_Lot,OP_BUY),clrBlack,clrBlack,false,false,false,0);
//----------------------------------------------------------------------------------------------
   objLabel("Lot@20",yy,xx+=xx_step,"Verdana",7,clrWhiteSmoke,
            "Sum Lots Sell ::");
//
//objLabel("Finance@12",yy+110,xx,"Verdana",7,clrNumberValue(ActiveBuy_Hold),
//         DoubleToStr(ActiveBuy_Hold,2));
   ...
   objEdit("Lot@21",0,DoubleToStr(ActiveSell_Lot,2)
           ,false,true,95,xx-3,70,15,"Verdana",7,ALIGN_RIGHT,CORNER_RIGHT_UPPER
           ,clrNumberValue(ActiveSell_Lot,OP_SELL),clrBlack,clrBlack,false,false,false,0);
//----------------------------------------------------------------------------------------------
   objLabel("Lot@30",yy,xx+=xx_step,"Verdana",7,clrWhiteSmoke,
            "Sum Lots        ::");
//
//objLabel("Finance@12",yy+110,xx,"Verdana",7,clrNumberValue(ActiveBuy_Hold),
//         DoubleToStr(ActiveBuy_Hold,2));
