      ...
      for(int i=0;i<=OrdersTotal();i++)
        {
         if(
            ...
            OrderSymbol()==Symbol() && 
            OrderType()<=OP_SELL && 
            ((_Magic==exMagicnumber && OrderMagicNumber()==_Magic)||
            (_Magic!=exMagicnumber))
            )
           {
            HaveOrders=true;
      ...
      for(int i=OrdersTotal()-1; i>=0; i--)
        {
         ...
         if(OrderSymbol()==Symbol() && OrderType()==OP_BUY && 
            ((iMN==exMagicnumber && OrderMagicNumber()==iMN)||
            (iMN!=exMagicnumber))
            )
           {
...
string EA_Remain(datetime DIFF)
  {
   ...
   string str="";
   if(_Year!=0) str+=string(_Year)+" Year ";
   if(_Month!=0) str+=string(_Month)+" Month ";
   if(_Day!=0) str+=string(_Day)+" Day ";
...
string OP_OrderToString(int v)
  {
   if(v==OP_BUY)    return "BUY";
   if(v==OP_SELL)   return "SEL";
   return "-";
  }
//+------------------------------------------------------------------+
