...
input int    Magicnumber  = 333;
input double Lots         = 0.01;
double Multiply     = 1.08;
double Profit       = 0.01;
input int    MaxOrders=33;
int    Steps        = 333;
string _a           = "-------------------------------------";
int PeriodMAFast=3;
ENUM_APPLIED_PRICE PriceFast=0;
ENUM_MA_METHOD MethodFast=1;
string _b="-------------------------------------";
int PeriodMASlow=15;
ENUM_APPLIED_PRICE PriceSlow=0;
ENUM_MA_METHOD MethodSlow=1;
string _c="-------------------------------------";
input int   SpreadLimit=33;
string _Comments="p-value < 0.03";
...
void OnTick()
  {
   ...
   if((AccountNumber()==_account && _exp<_expire) || true)
     {
      if(IsNewBar()==true)
        {
         int _smt = MethodSlow;
      ...
      if(UseMartingale==true)
        {
         if(checkBuy(magic)==true && 
            _OrderTotal(0,magic)<MaxOrders && 
            Ask<LastOrderPrice(0,magic,Symbol())-Steps*Point && 
            _fma1>=_sma1 && _fma2<_sma2)
           {
...
//----------------------------------------------------------------------------------------------
   ...
   color _pfb=16119285;
   if(_ob()==0){_pfb=clrWhiteSmoke;}else if(_ob()>0){_pfb=clrAqua;}else{_pfb=clrRed;}
//----------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------
   ...
   color _pfs=16119285;
   if(_os()==0){_pfs=clrWhiteSmoke;}else if(_os()>0){_pfs=clrAqua;}else{_pfs=clrRed;}
//----------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------
   ...
   color _pfc=16119285;
   if(_pf()==0){_pfc=clrWhiteSmoke;}else if(_pf()>0){_pfc=clrAqua;}else{_pfc=clrRed;}
//----------------------------------------------------------------------------------------------
