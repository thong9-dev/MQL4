...
//ChartSetInteger(0,CHART_SHOW_ASK_LINE,true);
   ...
   if(exMagic_Mode==MN_ALL)
      _Magic=Magicnumber*-1;
   else
      _Magic=Magicnumber;
//---
   if((exLot_C<-1.00 || exLot_C>1.00) && (exLot_Mode==LOT_Dynamic || exLot_Mode==LOT_Dynamic2))
     {
...
void OnTick()
  {
   if(EA_Expired())
     {
      if((ex2Lot_C!=-2 && exLot_Mode==LOT_Dynamic) || (exLot_Mode==LOT_Static || exLot_Mode==LOT_Dynamic2))
        {
            ...
            if(MarketInfo(Symbol(),MODE_SPREAD)<=exSpreadLimit)
              {
               ...
               if(vMA_Fast_1>=vMA_Slow_1 && vMA_Fast_2<vMA_Slow_2) //Fast cross up
                 {
                  if(ActiveBuy==0)
                     ticket=OrderSend(Symbol(),OP_BUY,NormalizeDouble(exLotStart,2),Ask,exSlipepage,0,0,_Comments,_Magic,0,clrBlue);
                  if(ActiveSell==0)
                     ticket=OrderSend(Symbol(),OP_SELL,NormalizeDouble(exLotStart*Multiply,2),Bid,exSlipepage,0,0,_Comments,_Magic,0,clrRed);
                 }

               if(vMA_Fast_1<=vMA_Slow_1 && vMA_Fast_2>vMA_Slow_2) //Fast cross down
                 {
                  if(ActiveBuy==0)
                     ticket=OrderSend(Symbol(),OP_BUY,NormalizeDouble(exLotStart*Multiply,2),Ask,exSlipepage,0,0,_Comments,_Magic,0,clrBlue);
                  if(ActiveSell==0)
                     ticket=OrderSend(Symbol(),OP_SELL,NormalizeDouble(exLotStart,2),Bid,exSlipepage,0,0,_Comments,_Magic,0,clrRed);
                 }
              }
         ...
         if(UseMartingale==true)
           {
            ...
            if(ActiveBuy>=1 && ActiveBuy<exMaxOrders && 
               Ask<(LastOrderPrice_Buy-(exSteps*Point*0.667)) && 
               vMA_Fast_1>=vMA_Slow_1 && vMA_Fast_2<vMA_Slow_2
               ) //Fast cross up
              {
               ticket=OrderSend(Symbol(),OP_BUY,Order_LotsMultiply(ActiveBuy,LastOrderPrice_Buy,Ask),Ask,exSlipepage,0,0,_Comments,_Magic,0,clrBlue);
              }
            //--------------------------------------------------------------------------------
            double LastOrderPrice_Sell=LastOrderPrice(OP_SELL,_Magic);

            if(ActiveSell>=1 && ActiveSell<exMaxOrders && 
               Bid>(LastOrderPrice_Sell+(exSteps*Point*0.667)) && 
               vMA_Fast_1<=vMA_Slow_1 && vMA_Fast_2>vMA_Slow_2
               )//Fast cross down
              {
               ticket=OrderSend(Symbol(),OP_SELL,Order_LotsMultiply(ActiveSell,LastOrderPrice_Sell,Bid),Bid,exSlipepage,0,0,_Comments,_Magic,0,clrRed);
              }
           }
         //---

         double _ActiveBuy_Lot=(ActiveBuy_Lot==0)?1:ActiveBuy_Lot;
         double _ActiveSell_Lot=(ActiveBuy_Lot==0)?1:ActiveBuy_Lot;

         double LotAvg=(_ActiveBuy_Lot>_ActiveSell_Lot)?_ActiveSell_Lot/_ActiveBuy_Lot:_ActiveBuy_Lot/_ActiveSell_Lot;
         LotAvg=1-LotAvg;

         if((ActiveBuy_Hold>=exProfit && ActiveSell_Hold>=exProfit) || 
            (LotAvg>=0.667 && Active_Hold>=exProfit*0.1) || 
            (ActiveBuy==ActiveSell && Active_Hold>=exProfit) || 
            (Active_Hold>=exProfit*3) || 
            (ActiveBuy_Hold>0 && ActiveSell_Hold<0 && Active_Hold>=exProfit) ||
            (ActiveBuy_Hold<0 && ActiveSell_Hold>0 && Active_Hold>=exProfit) ||
            (ActiveBuy_Hold>0 && ActiveSell_Hold>0 && Active_Hold>=exProfit*0.5) ||
            (ActiveBuy_Hold>=exProfit*0.25 && ActiveSell_Hold>=exProfit*0.25 && Active_Hold>=exProfit*2)
            )
           {
      ...
      for(int i=0;i<=OrdersTotal();i++)
        {
         if(
            ...
            OrderSymbol()==Symbol() && 
            OrderType()<=OP_SELL && 
            ((_Magic==exMagic_Mode && OrderMagicNumber()==_Magic)||
            (_Magic!=exMagic_Mode))
            )
           {
            HaveOrders=true;
...
//int exMode=1;
   ...
   if(exLot_Mode==LOT_Dynamic2)
     {
      lot=NormalizeDouble(MathAbs(Dy_Last-Dy_Now),Digits);
      lot=((lot/(exSteps*Point))+ex2Lot_C)*exLotStart*MathPow(Multiply,CNT);
      lot=NormalizeDouble(lot,2);
     }
      ...
      for(int i=OrdersTotal()-1; i>=0; i--)
        {
         ...
         if(OrderSymbol()==Symbol() && OrderType()==OP_BUY && 
            ((iMN==exMagic_Mode && OrderMagicNumber()==iMN)||
            (iMN!=exMagic_Mode))
            )
           {
...
//----------------------------------------------------------------------------------------------
   xx=35;
//---
   objLabel("EAname",yy,xx,"Verdana Bold",8,clrWhiteSmoke,EA_HaderName+" "+EA_Expired());
//----------------------------------------------------------------------------------------------
   ...
   if(EA_Expire!=-1)
     {
      ...
      datetime DayAlert=DayUnit*EA_DayAlert;         //Alert Ramian

      datetime Diff=datetime(EA_Expire)-TimeLocal();
      ...
      if((Diff<DayAlert && !EA_TimeAlertNext) || !event)
        {
         ...
         string msg=EA_HaderName+
                    ...
                    "\n Remain: "+EA_Remain(Diff,DayUnit);
         printf("Remain: "+string(event)+" "+EA_Remain(Diff,DayUnit));
...
//|                                                                  |
//+------------------------------------------------------------------+
string EA_Remain(datetime Diff,datetime DayUnit)
  {
   return string(TimeDay(Diff))+" Day "+string(TimeHour(Diff))+" Hr.";
//return TimeDay(Diff)+" Day "+TimeHour(Diff)+":"+TimeMinute(Diff);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
   ...
   for(int icnt=0;icnt<OrdersTotal();icnt++) // for loop
     {
      ...
      if(r && 
         ...
         )
        {
         ...
         if(Type==OP_BUY)
           {
            cntOP_BUY++;   aActiveBuy_Hold+=Hold;  aActiveBuy_Lot+=Lot;
         ...
         if(Type==OP_SELL)
           {
            cntOP_SELL++;  aActiveSell_Hold+=Hold; aActiveSell_Lot+=Lot;
