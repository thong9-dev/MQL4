...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: (Close(13) > ParabolicSAR(0.02, 0.2))
      if(TradeLong) {
         bool LongEntryCondition = (Close[13] > iSAR(NULL, 0, 0.02, 0.2, 1));
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: (Close(13) < ParabolicSAR(0.02, 0.2))
      if(TradeShort) {
         bool ShortEntryCondition = (Close[13] < iSAR(NULL, 0, 0.02, 0.2, 1));
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(iMA(NULL, 0, 56, 0, MODE_EMA, PRICE_CLOSE, 1) + (0.4) * (iATR(NULL, 0, 38, 1)));
   } else {
      // short
      return(iMA(NULL, 0, 56, 0, MODE_EMA, PRICE_CLOSE, 1) + (-0.4) * (iATR(NULL, 0, 38, 1)));
   }
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(240 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(240 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = Open[2] + (0.5) * (iATR(NULL, 0, 86, 1));
