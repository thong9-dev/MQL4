...
extern string CustomComment = "Strategy 5.46";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: MACD(12, 26, 9) > 0
      if(TradeLong) {
         bool LongEntryCondition = ruleMACD_Above();
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: MACD(12, 26, 9) < 0
      if(TradeShort) {
         bool ShortEntryCondition = ruleMACD_Below();
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 5.46", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(Low[1] + (0.3) * (iATR(NULL, 0, 54, 1)));
   } else {
      // short
      return(High[1] + (-0.3) * (iATR(NULL, 0, 54, 1)));
   }
}
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = High[2] + (0) * (iATR(NULL, 0, 4, 1));

   } else if (OrderType() == OP_SELL) {
      value = Low[2] + (0) * (iATR(NULL, 0, 4, 1));
   }
