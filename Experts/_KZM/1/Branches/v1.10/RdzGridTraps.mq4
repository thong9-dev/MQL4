...
void CreateOrders()
{
   ...
   for(int i = 0; i < CreationCount; i++)
   {
      ...
      {
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_BUYSTOP, DyStartingLots, CurrentUpperPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceBuy);
         if (Chk == -1)
         ...
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_SELLSTOP, DyStartingLots, CurrentLowerPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceSell);
         if (Chk == -1)
      ...
      {
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_SELLLIMIT, DyStartingLots, CurrentUpperPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceSell);
         if (Chk == -1)
            Alert(StringFormat("ERROR: %s, Lots: %s on Price: %s", GetErrorMessages("CreateOrders"), DoubleToString(DyStartingLots, 2), DoubleToString(CurrentUpperPrice, Digits)));
         else
         {
         ...
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_BUYLIMIT, DyStartingLots, CurrentLowerPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceBuy);
         if (Chk == -1)
            Alert(StringFormat("ERROR: %s, Lots: %s on Price: %s", GetErrorMessages("CreateOrders"), DoubleToString(DyStartingLots, 2), DoubleToString(CurrentLowerPrice, Digits)));
         else
         {
...
void SetComments()
{
   ...
   Cmt += Spacer + StringFormat("%s%s%s%s",
      ...
      (IsInStartTimeRange() ? " - IN TIMERANGE" : "")
      ) + CrLf;
   ...
   Cmt += Spacer + StringFormat("DAY-START: %s, DAY-END: %s", TimeToString(DailyStartDateTime, TIME_DATE | TIME_SECONDS), TimeToString(DailyStopDateTime, TIME_DATE | TIME_SECONDS)) + CrLf;
   Cmt += Spacer + StringFormat("WEEK-START: %s, WEEK-END: %s", TimeToString(WeeklyStartDateTime, TIME_DATE | TIME_SECONDS), TimeToString(WeeklyEndDateTime, TIME_DATE | TIME_SECONDS)) + CrLf;
