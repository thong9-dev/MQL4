...
struct stStartStopPointer
{
   ...
   bool Validate()
   {
      if (Start > 0 && Stop > 0 && Stop < Start) return true;
      return false;
   }
   
   bool IsInsideStopStart(datetime &currentDateTime)
   {
      if (Validate() && Stop < currentDateTime && currentDateTime < Start) return true;
      return false;
   }
};

struct stStartStopLines
{
   stStartStopPointer   Pointer[];
   int                  Count;
   int                  CorruptedCount;
   
   void Detect()
   {
      DetectStartStopLines();
   }
};
...
{
   ILTFixed = 0, //Fixed Standard
   ILTDynamic = 1, //Dynamic Standard
   ILTFixedGradual = 2, //Fixed Gradual
   ILTDynamicGradual = 3 //Dynamic Gradual
};
enum enDateTimeType
{
...
enum enOrderOperation
{
   ...
   FewLimitOrders = 2, //LIMIT orders then STOP orders (Experimental)
   AllBuyOrders = 3, //All Pending BUY Orders (Experimental)
   AllSellOrders = 4 //All Pending SELL Orders (Experimental)
};
enum enCloseMode
{
...
enum enStopTimeMode
{
   STMNothing = 0, //Inactive
   STMForceCloseNoOpenOrder = 1, //If none opened
   STMForceCloseAll = 2, //Just close unconditionally
   STMForceCloseProfit = 3, //If profit
   STMForceCloseProfitOrNoOpenOrder = 4 //If profit or none opened
};
...
enum enGradualOperator
{
   GOMultiply = 0, //Multiply
   GOAdd = 1 //Add
   
};
enum enTrailingProfit
{
   TRPNone = 0, //None
   TRPDragTrailing = 1 //Trailing (Drag)
};
enum enTrailingProfitStage
{
   TRPSInitial = 0, //Initial Stage
   TRPSTrailing = 1 //Trailing Stage
};
...
input          string                  StopNextCycleTagIFTTT      = "GT"; //Stop Next Cycle Tag for IFTTT.com
input          enStopTimeMode          StopTimeMode               = STMNothing; //Stop Time Action (Close Cycle)
...
input          enGradualOperator       GradualLotsOperator        = GOMultiply; //Gradual Lots Operator
input          double                  GradualLotsMult            = 1.5; //Gradual Lots Multiplier
input          string                  CommentInfo                = ""; //Comment Info
input          enProfitTakingMode      ProfitTakingMode           = PTMWithCommAndSwap; //Profit Taking Mode
input          enTrailingProfit        TrailingMode               = TRPNone; //Profit Trailing Mode
...
input          bool                    EnableNotification         = true; // Enable Notification
               ...
               int                     btnWidth                   = 100;
               int                     btnHeight                  = 30;
               string                  btnDefaultFont             = "Impact";
               int                     btnDefaultFontSize         = 10;
               ...
               double                  LastGradLots               = 0;
               double                  TrailingProfitDrag         = 0;
               string                  CrLf                       = "\n";
               string                  Space                      = " ";
               ...
               enTrailingProfitStage   TrailingStage              = TRPSInitial;

               stStartStopLines        StartStopLines;
...
int OnInit()
{
   ...
   DrawButton(btnForceCloseAll, btnForceCloseAllText, btnLeftAxis + btnInterval, btnTopAxis);
...
double OnTester()
{
   PrintFormat("HIGHEST OPEN ORDER COUNT: %s", IntegerToString(HighestOpenOrder));
                  ...
                  const string &sparam)
{
   ...
   if (id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      if (clickedObject == btnEnableTrading) //Stop Next Cycle
      {
         ...
         if (IsStopOnNextCycle()) //is set to stop
         {
            SetButtonText(btnEnableTrading, btnStopNextCycleText);
            SetButtonColor(btnEnableTrading, clrEnableTrading, DefaultButtonTextColor);
            SetStopNext(false);
            SetComments();
            TriggerTaggedEmailIFTTT(StopNextCycleTagIFTTT, StringFormat("Strategy '%s' is stopped.", CommentInfo));
         }
         else //is set to start
         {
            SetButtonText(btnEnableTrading, btnKeepGoingText);
            SetButtonColor(btnEnableTrading, clrDisableTrading);
            SetStopNext(true);
            SetComments();
            TriggerTaggedEmailIFTTT(StopNextCycleTagIFTTT, StringFormat("Strategy '%s' is running.", CommentInfo));
         }
      }
...
void OnTick()
{
   ...
   InTimeRange = IsInStartTimeRange();
   ...
   if (TrailingMode == TRPDragTrailing && TrailingStage == TRPSTrailing)
   {
      CalculateTrailingProfit();
   }
   ...
   if (
         (
            ...
            (StopTimeMode == STMForceCloseNoOpenOrder && (BuyCount + SellCount) == 0 && TimeExCodeStatus == TECSOutOfTimeRangeWaitEx) ||
            (StopTimeMode == STMForceCloseProfit && TotalProfit > 0 && TimeExCodeStatus == TECSOutOfTimeRangeWaitEx) ||
            (StopTimeMode == STMForceCloseProfitOrNoOpenOrder && (TotalProfit > 0 || (BuyCount + SellCount) == 0) && TimeExCodeStatus == TECSOutOfTimeRangeWaitEx)
         )
   ...
   if (!ActiveOrders && !IsStopOnNextCycle() && InTimeRange && (!EnableMaxCycle || (EnableMaxCycle && CycleCount < MaxCycle)))
   {
      CreateOrders();
   }
   ...
   if (!ActiveOrders)
   {
      DrawButton(btnStartTrading, btnStartManuallyText, btnLeftAxis + (btnInterval * 2), btnTopAxis);
   }
   else
   {
      DeleteButton(btnStartTrading);
...
{
   CycleCount = 0; //resetting the Cycle before OnInsideTimeRange executed.
}
...
void CalculateTrailingProfit()
{
   if (DyTargetProfit < (TotalProfit - TrailingProfitDrag))
   {
      DyTargetProfit = TotalProfit - TrailingProfitDrag;
   }
}
void CalculateStartingLots()
{
   ...
   if (InitialLotsType == ILTDynamic || InitialLotsType == ILTDynamicGradual)
   {
      double BaseCalc = 0;
   ...
   else if (InitialLotsType == ILTFixed || InitialLotsType == ILTFixedGradual)
   {
...
void CreateOrders()
{
   ...
   for(int i = 0; i < CreationCount; i++)
   {
      ...
      if ((InitialLotsType == ILTDynamicGradual || InitialLotsType == ILTFixedGradual) && !(i == 0 && InitialCreation))
      {
         if (GradualLotsOperator == GOAdd)
         {
            LastGradLots = LastGradLots + GradualLotsMult;
         }
         else if (GradualLotsOperator == GOMultiply)
         {
            LastGradLots = LastGradLots * GradualLotsMult;
         }
      }
      else if (InitialLotsType == ILTDynamic || InitialLots == ILTFixed)
      {
         LastGradLots = DyStartingLots;
      }
      
      OrderLotSize = NormalizeDouble(LastGradLots, 2);
      
      int Chk = 0;
      if (OrderOperation == StopOrders || (OrderOperation == FewLimitOrders && AllOrdersCount >= LimitOrdersCount))
      {
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_BUYSTOP, OrderLotSize, CurrentUpperPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceBuy);
         if (Chk == -1)
            Alert(StringFormat("ERROR: %s, Lots: %s on Price: %s", GetErrorMessages("CreateOrders"), DoubleToString(OrderLotSize, 2), DoubleToString(CurrentUpperPrice, Digits)));
         else
         {
         ...
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_SELLSTOP, OrderLotSize, CurrentLowerPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceSell);
         if (Chk == -1)
            Alert(StringFormat("ERROR: %s, Lots: %s on Price: %s", GetErrorMessages("CreateOrders"), DoubleToString(OrderLotSize, 2), DoubleToString(CurrentLowerPrice, Digits)));
         else
         {
      ...
      {
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_SELLLIMIT, OrderLotSize, CurrentUpperPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceSell);
         if (Chk == -1)
            Alert(StringFormat("ERROR: %s, Lots: %s on Price: %s", GetErrorMessages("CreateOrders"), DoubleToString(OrderLotSize, 2), DoubleToString(CurrentUpperPrice, Digits)));
         else
         {
         ...
         Chk = 0;
         Chk = OrderSend(Symbol(), OP_BUYLIMIT, OrderLotSize, CurrentLowerPrice, slp, PriceSL, PriceTP, CommentInfo, MagicNumber, 0, clrPlaceBuy);
         if (Chk == -1)
            Alert(StringFormat("ERROR: %s, Lots: %s on Price: %s", GetErrorMessages("CreateOrders"), DoubleToString(OrderLotSize, 2), DoubleToString(CurrentLowerPrice, Digits)));
         else
         {
...
bool IsTimeToClose()
{
   ...
   if (ActiveOrders)
   {
      ...
      if (TrailingMode == TRPDragTrailing && IsTargetReached && TrailingStage == TRPSInitial)
      {
         TrailingStage = TRPSTrailing;
         if (TargetType == TTFixed)
         {
            TrailingProfitDrag = TargetProfit;
         }
         else if (TargetType == TTDynamic)
         {
            TrailingProfitDrag = DyTargetProfit;
         }
         DyTargetProfit = 0;
         IsTargetReached = false; //set the target back to false,assuming the Trailing starts here
      } else if (TrailingMode == TRPDragTrailing && TrailingStage == TRPSTrailing && TotalProfit <= DyTargetProfit)
      {
      ...
      if (
         ...
         (CloseType == CTThresholdAllOrdersExtd && (
            IsTargetReached || IsLossAccepted || IsReachingThreshold || (BuyCount == SellCount && BuyCount > 0 && BuyCount == DyOrderCountPerSide)))
      )
      {
         IsTime = true;
      }
...
bool IsInStartTimeRange()
{
      ...
      )
   {
      ...
      {
         IsTime = !IsInsideStartStopTimes();
      }
   }
   return IsTime;
}
bool IsInsideStartStopTimes()
{
   datetime CurrentServerTime = TimeCurrent();
   for (int i = 0; i < StartStopLines.Count; i++)
   {
      if (CurrentServerTime >= StartStopLines.Pointer[i].Stop && CurrentServerTime <= StartStopLines.Pointer[i].Start)
      {
         return true;
         break;
      }
...
{
   bool act = ActiveOrders;
   NoCorruptedCheck = true;
   
   if (HighestOpenOrder < (BuyCount + SellCount)) HighestOpenOrder = (BuyCount + SellCount);
   ...
   */
   ...
   if (EnableNotification && act)
   {
      string NotificationMsg = StringFormat("[RdzGridTraps] %s: %s%s on %s - %s (%s%s - %s)",
...
void CloseOrdersOutsideIn(enCloseOrdersMode CloseOrdersMode = COMDefault)
{
   ...
   for (int i = OrdersTotal(); i >= 0; i--)
   {
      ...
      RefreshRates();
      Chk = OrderSelect(i, SELECT_BY_POS);
      if (Chk > 0 && OrderMagicNumber() == MagicNumber && OrderCloseTime() == 0)
      {
...
void SetComments()
{
   ...
   Cmt += CrLf + CrLf + CrLf + CrLf;
   
   if (StringLen(CommentInfo) > 0)
   {
      Cmt += Spacer + StringFormat("INFO: %s", CommentInfo) + CrLf;
   }
   Cmt += Spacer + StringFormat("MAGIC:%s - %s - MODE:%s - TIMERANGE:%s",
      IntegerToString(MagicNumber),
      (ActiveOrders ? "ACTIVE" : "INACTIVE"),
      (IsStopOnNextCycle() ? "STOPNEXT" : "ALWAYS"),
      (IsInStartTimeRange() ? "IN" : "OUT")
      ) + CrLf;
   ...
   else if (InitialLotsType == ILTDynamicGradual)
   {
      Cmt += Spacer + StringFormat("DyLot from: %s %s by %s", DoubleToString(DyStartingLots, 2), (GradualLotsOperator == GOMultiply ? "multiplied" : "added"), DoubleToString(GradualLotsMult, 5)) + CrLf;
   }
   else if (InitialLotsType == ILTFixedGradual)
   {
      Cmt += Spacer + StringFormat("FxLot from: %s %s by %s", DoubleToString(DyStartingLots, 2), (GradualLotsOperator == GOMultiply ? "multiplied" : "added"), DoubleToString(GradualLotsMult, 5)) + CrLf;
   }
   if (LossType == LTDynamic)
   {
   ...
   Cmt += CrLf;
   Cmt += Spacer + StringFormat("NOW: %s", TimeToString(CurrentTime, TIME_DATE | TIME_SECONDS)) + CrLf;
   Cmt += Spacer + StringFormat("DAY START: %s, END: %s", TimeToString(DailyStartDateTime, TIME_DATE | TIME_SECONDS), TimeToString(DailyStopDateTime, TIME_DATE | TIME_SECONDS)) + CrLf;
   Cmt += Spacer + StringFormat("WEEK START: %s, END: %s", TimeToString(WeeklyStartDateTime, TIME_DATE | TIME_SECONDS), TimeToString(WeeklyEndDateTime, TIME_DATE | TIME_SECONDS)) + CrLf;
   
   if (UseLineStartStop)
   {
      Cmt += Spacer + StringFormat("LINESREAD: %s pairs, CORRUPTED: %s pairs", IntegerToString(StartStopLines.Count), IntegerToString(StartStopLines.CorruptedCount)) + CrLf;
      if (StartStopLines.Count > 0)
      {
         Cmt += Spacer + StringFormat("CURRENT STOP: %s, START: %s", TimeToString(StartStopLines.Pointer[0].Stop, TIME_DATE | TIME_SECONDS), TimeToString(StartStopLines.Pointer[0].Start, TIME_DATE | TIME_SECONDS)) + CrLf;
      }
   }
...
bool FindObject(string ctlName)
{
...
void ObjectButton(string ctlName, enObjectOperation Operation, string Text = "", int X = -1, int Y = -1, int Width = -1, int Height = -1, bool Selected = false, color BgColor = clrNONE, color TextColor = clrNONE)
{
   ...
   if ((ObjectFind(ChartID(), ctlName) > -1))
   {
      if (Operation == LODraw)
      {
         ...
         ObjectSetString(ChartID(), ctlName, OBJPROP_FONT, btnDefaultFont);
         ObjectSetString(ChartID(), ctlName, OBJPROP_TEXT, Text);
         ObjectSetInteger(ChartID(), ctlName, OBJPROP_FONTSIZE, btnDefaultFontSize);
...
datetime GetLineDateTime(string ctlName)
{
   datetime TheDateTime = 0;
   string FullCtlName = ctlName;
   if (ObjectFind(ChartID(), FullCtlName) > -1)
   {
      TheDateTime = (datetime)ObjectGetInteger(ChartID(), FullCtlName, OBJPROP_TIME);
   }
   return TheDateTime;
}
void DetectStartStopLines() //it must use Server Time only
{
   int i = 0;//index
   int cc = 0; //corrupted count
   
   ArrayResize(StartStopLines.Pointer, 0, 100); //resets the pointer
   ...
   while (true)
   {
      ...
      cc += 1;
      int LastPointerSize = ArraySize(StartStopLines.Pointer);
      
      string sStartCtlName = StringConcatenate(LineStartStopKeyword, "START", IntegerToString(i));
      string sStopCtlName = StringConcatenate(LineStartStopKeyword, "STOP", IntegerToString(i));
      
      PrintFormat("Stop: %s, Start: %s", sStopCtlName, sStartCtlName);
      
      if (FindObject(sStartCtlName) && FindObject(sStopCtlName)) //if both object at current index are found
      {
         datetime dtStart = GetLineDateTime(sStartCtlName);
         datetime dtStop = GetLineDateTime(sStopCtlName);
         
         PrintFormat("STOP:%s START:%s", TimeToString(dtStop, TIME_DATE | TIME_SECONDS), TimeToString(dtStart, TIME_DATE | TIME_SECONDS));
         
         if (dtStart > 0 && dtStop > 0 && dtStop < dtStart) //to check that the value read should be in proper format
         {
            if ((LastPointerSize > 0 && (dtStart > StartStopLines.Pointer[LastPointerSize - 1].Stop)) || LastPointerSize == 0)
            {
               ArrayResize(StartStopLines.Pointer, LastPointerSize + 1, 100);
               StartStopLines.Pointer[LastPointerSize].Start = dtStart;
               StartStopLines.Pointer[LastPointerSize].Stop = dtStop;
               cc -= 1;
            }
         }
      }
   ...
   StartStopLines.Count = ArraySize(StartStopLines.Pointer);
   StartStopLines.CorruptedCount = cc;
}

void TriggerTaggedEmailIFTTT(string Tag, string BodyMessage)
{
   bool bRet = false;
   
   string Subject = "#" + Tag + " [RdzGridTraps]";
   string Body = BodyMessage + CrLf + CrLf + "Sent via RdzGridTraps EA (http://rdzgridtraps.codeplex.com)";

   bRet = SendMail(Subject, Body);
}
