...
enum enCloseType
{
   ...
   CTThreshold = 3 //Threshold or Target Based
};
enum enOpenType
{
...
input          bool                    UseGVStartStop             = false; //Use Start and Stop from Global Variable
input          string                  GVStartStopKeyword         = "GT"; //Start and Stop Global Variable Keyword
...
input          bool                    LimitMaxOrderCount         = false; //Enable Max Order Count Limiter
input          int                     MaxOrderCount              = 0; //Maximum Order Count (total)
...
//input          string                  LotsSeriesMult             = "1"; // Lots Series Multiplier
...
input          int                     MiddlePointRange           = 5; // Middle Point Range (from Ask and Bid)
input          int                     CloseMiddlePointAftOrders  = 0; //Close Orders After X (Start Points)
...
void OnTick()
{
   ...
   if (!ActiveOrders)
   {
      DrawButton(btnStartTrading, btnStartManuallyText, 30, 270);
   }
   else
   {
      DeleteButton(btnStartTrading);
   ...
   if (ActiveOrders)
   {
      ...
      if (
         ...
         (CloseType == CTThreshold && (
            IsTargetReached || IsLossAccepted || AddPoints(LastUpperPrice, 0 - GridStepPoints - CloseThreshold) <= Ask || AddPoints(LastLowerPrice, GridStepPoints + CloseThreshold) > Bid))
      )
      {
         IsTime = true;
      }
...
{
   int ordticket[];
   ArrayResize(ordticket, OrdersTotal(), 1000);
...
void SetComments()
{
   ...
   Cmt += Spacer + StringFormat("CP: %s %s", DoubleToString(CurrentProfit, 2), Currency) + CrLf;
   ...
   if (EnableMaxCycle)
   {
      Cmt += Spacer + StringFormat("Cyc: %s, Max: %s", IntegerToString(CycleCount), IntegerToString(MaxCycle)) + CrLf;
   }
