   ...
   if(ArraySize(TroopB_PTP)>2 && strCMMResult_bool)
     {
      ...
      Text=c(Q_D,Digits)+"P ~ "+c(Q_Lot,2)+"L | "+c(Q_Cap+Magin,2)+"C | "+c(Magin,2)+"m";
      ...
      //---
      double U_Lot=(Capital_A*T1)/(D*CONSIZE),U_D=0;
      printf(0);
      ...
      Q_Cap_Per=(Capital_A/Arr_Cap[Digits-1])*100;
      Q_Cap_Per=NormalizeDouble(Q_Cap_Per,2);

      Text="** Put money now **";
      y+=10;LabelCreate(0,ExtName_OBJ+"PutTageUse1",0,x+10,y,0,Text,"",fontSize,clrTomato,0,false,false,false,0);

      Text=c(U_D,Digits)+"P ~ "+c(U_Lot,2)+"L | "+c(Capital_A,2)+"C | "+c(Q_Cap_Per,2)+"% "+c((ArraySize(TroopB_PTP)-1)*Capital_A,2);
               ...
               for(int j=0;j<int(1/Management_ZoneB_getConstant());j++)
                 {
                  ...
                  HLineCreate_(0,ExtName_OBJ+"DRAFT_ZONE_B"+c(j),"",0,P_Mark,color(_clrLineZoneB),3,0,true,false,false,0);
                 }
              }
         ...
         if(!boolAutoPending)
           {
            if(DeadLine>0)
              ...
              {
               PlaceTrade=MessageBox("DeadLine : "+c(DeadLine,Digits)+"\nDeadLine Not done",NameBTN+l(__LINE__),MB_OK|MB_OKCANCEL|MB_ICONWARNING);
              }
           }
         else
           {
   ...
   if(sparam==NameBTN)
     {
      ...
      if(PlaceTrade==IDOK)
        {
         double lot=NormalizeDoubleCut(LotGet(Capital_B,Ask,DeadLine),2);
         if(lot>0)
           {
            bool Send_=OrderSend(Symbol(),OP_BUY,lot,Ask,Slippage,0,0,"H_"+c(Capital_B,2)+l(__LINE__),2,0);
           }
        }
     }
  }
...
//---
   ...
   ObjectSetText(ExtName_OBJ+"Head_7","Auto",10,"Arial",clrWhite);
   ...
   PostX=PostX_Default+(Size_Wide+5)*7;PostY=PostY_Default;
