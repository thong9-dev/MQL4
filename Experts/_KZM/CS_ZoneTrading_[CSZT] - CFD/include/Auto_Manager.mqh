...
/*if((TP_UseATR && TP_UseATR)==false)
      ...
      lot=LotGet(Capital_A,TroopA[i],DeadLine);
               ...
               if(OrderLots_b || BASE_b)
                 {
                  ...
                  if(TroopA[i]>Ask && OP_HaveInDock<=1)
                    {
                     ResetLastError();
                     Send_=OrderSend(Symbol(),OP_BUYSTOP,lot,TroopA[i],Slippage,0,TP,Spector+c(i)+"_PN_"+c(Capital_A,2)+l(__LINE__),OrderMagic,0);
                     ...
                     if(checkError==ERR_NO_ERROR)
                       ...
                       {
                        ResetLastError();
                        Send_=OrderSend(Symbol(),OP_BUY,lot,Ask,Slippage,0,TP,Spector+c(i)+"_AC_"+c(Capital_A,2)+l(__LINE__),OrderMagic,0);
                        checkError=GetLastError();
                        if(checkError==ERR_NO_ERROR)
                           OP_HaveInDock++;
                       }
                    ...
                    {
                     ResetLastError();
                     Send_=OrderSend(Symbol(),OP_BUYLIMIT,lot,TroopA[i],Slippage,0,TP,Spector+c(i)+"_PN_"+c(Capital_A,2)+l(__LINE__),OrderMagic,0);
                     checkError=GetLastError();
                     if(checkError==ERR_NO_ERROR)
                        OP_HaveInDock++;
                     else if(checkError!=ERR_NO_ERROR && (TroopA[i]<=Ask && Ask<TroopA[i+1]))
   ...
   for(int i=0;i<ArraySize(TroopB_PTP)-1;i++)
     {
      ...
      //---
      if(Calculate_Margin)
         lot=LotGet_Magin(Capital_B,Pin,DeadLine);
      else
         lot=LotGet(Capital_B,Pin,DeadLine);
...
//double Magin=getMagin(double bid,double lots);
   double D=NormalizeDouble((Traget_2-Traget_1)*MathPow(10,Digits),0);
   ...
   printf("#"+__LINE__+" LotGet* cap_: "+cap_+" | D: "+D+" | Lot: "+Lot);
